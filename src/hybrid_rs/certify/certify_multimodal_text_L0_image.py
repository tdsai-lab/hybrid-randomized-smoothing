#!/usr/bin/env python3
# ============================================================
# TRUE Hybrid RS (text L0 + image L2) — NOT wasteful version
#
# Text threat: L0_inplace (attacker may change <= d tokens anywhere)
# Text smoothing kernel K (independent across positions):
#   each token is replaced with prob beta by Unif(allowed \ {t})
# Image smoothing: Gaussian N(0, sigma^2 I) in pixel space (or diffusion)
#
# Key optimization:
#   Since the text kernel K does NOT depend on d, pA is estimated ONCE per example,
#   then reused for all d in {1..d_max_eff}. Only the NP solver varies with d.
# ============================================================
...
def noise_uniform_l0_full_inplace_ids(tokenizer, text: str, beta: float, allowed_ids: List[int]):
    """
    Kernel K:
      For each token position i (of the ORIGINAL text length):
        with prob 1-beta keep token,
        with prob beta replace uniformly in allowed_ids without {t_i}.
    """
    ...
    noised_text = tokenizer.decode(out, skip_special_tokens=True, clean_up_tokenization_spaces=False)
...
# ------------------------------------------------------------
# MC estimate pA ONCE (kernel does not depend on d)
# ------------------------------------------------------------
@torch.no_grad()
def estimate_pA_lcb_once(
    ...
    pbar.set_description("MC pA (once)")
    ...
    while accepted < int(n):
        ...
        if use_diffusion:
            ...
            xhat_bchw = denoiser(x0_bchw, float(sigma))
            xhat_bchw_cpu = xhat_bchw.detach().cpu()
            images_for_proc = [to_pil_image(xhat_bchw_cpu[k]) for k in range(b)]
        else:
            images_for_proc = [add_gaussian_noise(image0, float(sigma)) for _ in range(b)]
        ...
        inputs = detector.processor(text=prompts, images=images_for_proc, return_tensors="pt", padding=True)
    ...
    pbar.close()
    pA_lcb = clopper_pearson_lcb(int(n), int(nA), float(tau_alpha))
...
def main():
    ...
    parser.add_argument("--text_threat", choices=["l0_inplace"], default="l0_inplace")
    ...
    parser.add_argument("--out", type=str, default="./results/true_hybrid_rs_l0_loopd.json")
    ...
    args.out = os.path.join(
        "results_l0",
        f"true_hybrid_rs_l0_loopd_{args.kernel}_b{args.beta}_s{args.sigma}_n{args.n}_max_exemples{args.max_examples}_diff_{args.use_diffusion}_dataset_{args.dataset}.json",
    )
    ...
    for i, ((text0, image0), _) in enumerate(loader):
        ...
        # ---- estimate pA once for this example ----
        pA_lcb, nA_unsafe = estimate_pA_lcb_once(
            detector=detector,
            text0=text0,
            image0=image0,
            allowed_ids=allowed_ids,
        ...
        # ---- loop over d: NP certificate only (cheap) ----
        for d in range(1, d_max_eff + 1):
            if args.kernel == "uniform":
                pc, pa, g = grouped_uniform(int(d), float(args.beta), int(V_eff))
            else:
                pc, pa, g = grouped_absorb(int(d), float(args.beta))
            ...
            print("Example %d | d=%d | pA_lcb=%.4f | nA_unsafe=%d | r_d=%.4f" % (i, d, pA_lcb, nA_unsafe, r_d))
